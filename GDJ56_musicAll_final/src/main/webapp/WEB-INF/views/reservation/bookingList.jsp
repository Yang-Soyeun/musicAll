    ...
    <br><br><br>
    	...
    	<c:forEach var="p" items="${musicalPhoto}">
    		<c:if test="${empty loginMember  }">
    			<div class="photo-box">
        			<img src="${path }/resources/upload/performance/${p.I_NAME}" class="perView" style="float:left; margin-top:30px;" onclick="location.href='${path}/booking/bookingview.do?mCode=${p.M_CODE }'" >
   				</div>
   			</c:if>
     		<c:if test="${not empty loginMember  }">
    			<div class="photo-box">
        			<img src="${path }/resources/upload/performance/${p.I_NAME}" class="perView" style="float:left; margin-top:50px;" onclick="location.href='${path}/booking/bookingview.do?mCode=${p.M_CODE }'" >
   				</div>
    		</c:if>
    	</c:forEach> 
    	</div>
...
//체크박스 장르 가져오기
	function getCheckboxValue(event)  {
		...
		console.log(result);
		if(result=='오리지널')	{
			location.assign("${path}/booking/searchForType.do?mType=1");
		}
		if(result=='라이센스')	{
			location.assign("${path}/booking/searchForType.do?mType=2");
		}
		if(result=='창작뮤지컬')	{
			location.assign("${path}/booking/searchForType.do?mType=3");
		}
		if(result=='넌버벌')	{
			location.assign("${path}/booking/searchForType.do?mType=4");
		}
		if(result=='아동')	{
			location.assign("${path}/booking/searchForType.do?mType=5");
		}
		}else{
		result = '';
		}
